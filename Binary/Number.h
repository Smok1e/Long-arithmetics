...
public:
	...
	Number <storage_t>& operator >>= (int shift);
...
template <class storage_t>
Number <storage_t>& Number <storage_t>::operator >>= (int shift)
{
	size_t size = this -> size ();
	for (size_t i = 0; i < size; i++)
	{
		...
		if (i < size)
			operator[] (size-i-1) = (inv-shift >= 0 && inv-shift < size)? operator[] (inv-shift): 0;
	}
